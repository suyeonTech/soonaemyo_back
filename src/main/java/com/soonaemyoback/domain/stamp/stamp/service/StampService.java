...
public class StampService {
    ...
    public List<MemberStampResponse> getMemberStampList(String name, Integer year, Integer semester) {
        if (name == null || name.isBlank()) {
            throw new IllegalArgumentException("이름을 입력해주세요.");
        }
        return stampRepository.searchStamps(year, semester, name).stream()
                .map(MemberStampResponse::from)
                .toList();
    }

    public List<StampSummaryResponse> getStampList(Integer year, Integer semester, String name) {
        return stampRepository.searchStamps(year, semester, (name != null && name.isBlank()) ? null : name).stream()
    ...
    public StampSummaryResponse giveStamp(Long stampId, StampKind stampKind, Long adminId) {
        ...
        giveStampRepository.save(GiveStamp.create(member, admin, stampKind.name()));
